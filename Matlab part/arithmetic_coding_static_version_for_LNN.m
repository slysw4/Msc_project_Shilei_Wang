...
cum_counts = [0, ceil(cumsum(counts)/size(input_data,1)) ];

% generate different count table for different dynamic range and binning
% period
for i = 1:(length(cum_counts)-1)
    

    if(cum_counts(i) > (cum_counts(end)- length(cum_counts) + i) )
        cum_counts(i) = cum_counts(i) - length(cum_counts) + i;
    end 

    while (cum_counts(i) > cum_counts(end)- length(cum_counts) + i )
    ...
    for i = 1:(length(cum_counts)-1)
       if (cum_counts(i) >= cum_counts(i+1))
         cum_counts(i+1) = cum_counts(i+1) + 1;
       end 
    end 
% cum_counts =[0,30,43,45,46,47,48];
...
for iteration = 1:size(input_data,1) 
        ...
        for k = 1:size(input_values,1)

             symbol_index = find(symbols == input_values(k));

            code_range = upper_bound-lower_bound+1;
        
            % Compute the new upper bound
            upper_bound = lower_bound + floor( code_range*cum_counts(symbol_index+1)/total_count )-1;
        
            % Compute the new lower bound
            lower_bound = lower_bound + floor( code_range*cum_counts(symbol_index+1-1)/total_count );
